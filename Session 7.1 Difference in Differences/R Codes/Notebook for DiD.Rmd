...
simple_did_descstats <- simple_did %>%
  ...
  summarize(mean_lncinc_pcr = mean(lninc_pcr, na.rm=TRUE))

simple_did_descstats %>%
  show_table()#note missing values for 2015 (STATA automatically drops these)


```
```{r}
cat("(7.913204-5.914218)-(7.433907-5.927306) #simple DiD \n", "Impact = ", (7.913204-5.914218)-(7.433907-5.927306))
...
did_felm1 <- felm(lninc_pcr ~ factor(cityx) + factor(year)  | psgc | 0 | prv, simple_did)
summary(did_felm1)

#to replicate computation of intercept (average value of fixed effects) in STATA:
did_reg2 <- plm(lninc_pcr ~ factor(year) + factor(cityx),
...
did_felm2 <- felm(lninc_pcr ~ factor(year) + factor(cityx) + lninclo_r3 + lnpopx3 | psgc | 0 | prv, simple_did)
summary(did_felm2)
#to replicate computation of intercept (average value of fixed effects) in STATA:
did_reg3 <- plm(lninc_pcr ~ factor(year) + factor(cityx) + lninclo_r3 + lnpopx3,
                simple_did, index = c("psgc", "year"), model = "within")
...
stargazer(felm_nolags, felm_withlags, type = "html", keep = "cityx3",
...
stargazer(felm_nolags2, felm_withlags2, type = "html", keep = "city_rand",
          dep.var.labels=c("No Lags", "With Lags"),
          column.labels = as.character(outcome_list))
```
...
stargazer(felm_nolags3, felm_withlags3, type = "html", keep = "cityx")
```
